...
export default function ClaimsPage() {
  ...
  async function load() {
    setError('')
    try {
      const endpoint = user.role === 'student' ? '/me/claims' : '/claims'
    ...
    <div style={{ background: '#fafafa' }} className="min-h-screen w-full pb-20">
      <main className="max-w-[1300px] mx-auto px-6 lg:px-8 py-8 animate-in">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 gap-4">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-widest text-[#16a34a] mb-1">Quản lý</p>
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-800">Ghi nhận / Claims</h1>
                  ...
                  <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-2">
                        {c.evidence_url ? (
                          <a href={c.evidence_url} target="_blank" className="w-10 h-10 rounded-lg overflow-hidden border border-slate-200 block shadow-sm hover:ring-2 hover:ring-[#16a34a]/30 transition-all">
                    ...
                    <td className="p-4 text-center">
                      {user.role !== 'student' && c.status === 'submitted' ? (
                        <div className="flex justify-center gap-2">
                           <button onClick={() => handleApprove(c.id)} disabled={busy === c.id} className="text-[#16a34a] hover:bg-green-50 p-1.5 rounded-md transition-colors"><span className="material-symbols-outlined text-[18px]">check</span></button>
                           <button onClick={() => handleReject(c.id)} disabled={busy === c.id} className="text-red-500 hover:bg-red-50 p-1.5 rounded-md transition-colors"><span className="material-symbols-outlined text-[18px]">close</span></button>
                        </div>
                      ) : (
